...
MANIFEST = MODEL_DIR / "smp_v36_sd_seed20261902_gate5999_93d.manifest.json"
PROFILE = "v36_sd_seed20261902_gate5999"


class SmpV36SdDeploymentTest(unittest.TestCase):
    def test_rd_remains_default_and_v36_requires_explicit_selection(self):
    ...
    def test_model_manifest_and_runtime_interface(self):
        ...
        self.assertEqual(cfg["profile"], "v35_rd_gate5999")
        ...
        self.assertEqual(manifest["source_wandb_run"], "tabletennis/smp/2yehj243")
        self.assertEqual(manifest["source_arm"], "SD")
        self.assertEqual(manifest["source_policy_seed"], 20261902)
        self.assertEqual(manifest["source_checkpoint_iteration"], 5999)
        self.assertEqual(manifest["evaluation"]["num_rollouts"], 4096)
        self.assertAlmostEqual(
            manifest["evaluation"]["macro_strict_success_rate"], 0.986328125
        )
        self.assertFalse(manifest["evidence_status"]["hardware_authorized"])
        self.assertFalse(manifest["runtime_safety"]["default_profile"])

...
FREEKICK_ONNX="${FREEKICK_ONNX:-${REPO_DIR}/policy/robonaldo/model/policy-obs-utf8-486.onnx}"
...
tmux send-keys -t "${RIGHT_TOP}"    -l "cd ${REPO_DIR} && { pkill -f 'onboard/perception/camera/target_ball_detector.py.*--v4l2-device /dev/video3' || true; pkill -f 'onboard/perception/camera/_launch.sh.*--v4l2-device /dev/video3' || true; if command -v fuser >/dev/null 2>&1; then fuser -k 8080/tcp || true; fuser -k /dev/video3 || true; fi; } && ./onboard/perception/camera/run_gray.sh --with-fuser --show"
